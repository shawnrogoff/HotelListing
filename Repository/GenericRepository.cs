...
public class GenericRepository<T> : IGenericRepository<T> where T : class
{
    ...
    {
        IQueryable<T> query = _db; // Get all the records from this table/dbSet
        if (includes != null)
        {
            foreach (var includeProperty in includes)
            {
                query = query.Include(includeProperty); // this would return foreign key tables as well
            }
        }
    ...
    {
        _db.Attach(entity); // Pay attention, and see if you have it already in the db
